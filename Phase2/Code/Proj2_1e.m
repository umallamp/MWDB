...
directory = strcat(dirPath,'\difference');

% Reading file numbers from user
prompt = 'Please enter a difference file number:';
f1 = input(prompt,'s');
prompt = 'Please enter a difference file number:';
f2 = input(prompt,'s');
...
str = strcat('The similarity between epidemic difference files',' ',f1,' and',' ',f2,' -->',' ',num2str(similarityVal));
disp(str);
